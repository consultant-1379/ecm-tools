...
def monitor_run(log_message, function_to_run):
    ...
    logger.info("Running '%s' on deployment: '%s'.", function_to_run.__name__, enmaas_conf.name)
    log_header(log_message.format(enmaas_conf.name))
    function_to_run(enmaas_conf.name)
...
def check_stack_health(deployment_name):
    """
    ...
    for stack in stacks:
        ...
        # Skip all stacks not related to the current tenancy
        # This change is done to accomodate new changes to OSP 13 stack list behaviour
        if deployment_name.lower() not in stack_name.lower():
            continue
        ...
        for key, value in stack_parameters.iteritems():
            ...
            # adding two keywords to skip on 22nov2019 as it is not used now : udc dashboard, lvs_advert
            parameters_to_skip = ['ipv6', 'external_mtu', 'external_share_ip_list', 'udcdashboard', 'lvs_advert', 'availability_zone']
...
def check_nested_stack_status(deployment_name):
    """
    ...
    for each in dictoutput:
        if deployment_name.lower() not in each["Stack Name"].lower():
            continue
...
def check_volume_attachments_status(deployment_name):
    """
    ...
    for stack in stacks:
        ...
        if deployment_name.lower() not in current_stack_name.lower():
            continue
