...
def get_onsite_backups_size(valid_backups_folders, source_dir, block_size):
    ...
    for backup_folder in valid_backups_folders:
        ...
        valid_value, log_message, backup_value = get_size_on_disk(valid_backup_dir_path)
...
def get_size_on_disk(file_or_dir_path):
    """
    Get the backup size on disk.

    :param file_or_dir_path: the full file or directory path on disk.
    :return: tuple: true, empty message to be ignored, the valid file/dir size.
             or
             tuple: false, exception message, -1, in case of any failure.
    """
    du = Popen(["du", "-sm", file_or_dir_path], stdout=PIPE)
    splitline = ""
    ...
    try:
        ...
        log_message = ("Exception while getting the size on disk, for the path: {}, "
                       "exception message: {}"
                       .format(file_or_dir_path, str(e)))
        return False, log_message, -1
...
def log_volume_size_before_transfer(vol_path_bkp_tmp, logger):
    """
    log the backups size in bkp_tmp for the customer has finished processing, before transferring.

    :param vol_path_bkp_tmp: the volume path under bkp_tmp directory.
    :param logger: passed logger object from the caller.
    """
    valid_value, log_message, vol_size = get_size_on_disk(vol_path_bkp_tmp)

    if valid_value:
        if vol_size >= 1024:
            vol_size = vol_size / 1024  # convert to GB for readability.
            block_size = "GB"
        else:
            block_size = "MB"

        log_message = ("The volume size at path: {},  is: {} {}".format(vol_path_bkp_tmp,
                                                                        vol_size, block_size))
